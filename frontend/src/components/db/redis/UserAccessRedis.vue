    ...
    data () {
        return {
            ...
            rules: {
                ...
                redisinst: [
                    { required: true, message: '请选择Redis实例', trigger: 'change' }
                ],
            },
            isapply: false,
        }
    ...
    methods: {
        ...
        handleApply() {
            this.$refs['form'].validate((valid)=>{
                if(valid){
                    addUserAccessRedis(this.form).then((response) => {
                        if (response) {
        ...
        delData(id) {
            delUserAccessRedis({id:id}).then((response) => {
        ...
        editData (data) {
            editUserAccessRedis(data).then((response) => {
        ...
        searchData() {
            if (this.searchcontent) {
                searchUserAccessRedis({searchcontent:this.searchcontent}).then((response) => {
        ...
        getDataList() {
            getUserAccessRedis({'username':store.getters.username}).then((response) => {
                if (response) {
                    this.table_data = response.data
                }
            }).catch((error) => {
...
<style>
.useraccessredis .el-collapse-item__header.is-active,.useraccessredis .el-collapse-item__header{
    font-size: 16px;
    font-weight: bold;
    padding-left: 0.5em;

                ...
                <el-form :model="form" :rules="rules" ref="form"> 
                    ...
                    <el-form-item label="用户组 : " label-width="100px" prop="group">
                        <el-radio-group v-model="form.group">
                            <el-radio v-for="(val,index) in usergrouplist" :key="index" :label="val.group_name"></el-radio>
                        </el-radio-group>
                    </el-form-item>
...
<script>
import { getUserGroupList } from '@/api/api.js';
export default {
    name: 'userdialog',
    data () {
        return {
            ...
            getUserGroupList().then((response) => {
                if (response.data.length > 0) {
                    for (var i = 0;i < response.data.length; i++) {
                        this.usergrouplist.push({'id':response.data[i].id,'group_name':response.data[i].group_name})
                    }
                }
            }).catch((error) => {
                console.log(error);

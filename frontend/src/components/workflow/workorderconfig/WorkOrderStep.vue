...
<script>
import { getApprovalGroup,addWorkOrderStep} from '@/api/api.js';
export default {
    name: 'workorderstep',
    data() {
        return {
    ...
    methods: {
        handleAddStep() {
            ...
            addWorkOrderStep(this.stepform).then((response) => {
                this.isadd = false;
                this.getWorkOrderTypeFun(this.ordertypeid)
            })
        },
